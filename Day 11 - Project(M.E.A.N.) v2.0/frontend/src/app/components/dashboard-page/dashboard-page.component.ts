...
export class DashboardPageComponent implements OnInit {
  ...
  approveAndDownload(){
    //alert('Here we download file and approve it through fancy shmancy CVS format.') 
    //sessionStorage.setItem('globalFROM', start.format('YYYY-MM-DD'))
    console.log('->' + sessionStorage.getItem('globalFROM'))
    console.log('->' + sessionStorage.getItem('globalTO'))
    let from = sessionStorage.getItem('globalFROM')
    let to = sessionStorage.getItem('globalTO')
    let tempData : Project[] = [];
    this.projectService.getProjectsByDate(from!,to!).subscribe(
      data =>{
        tempData = data;
        console.log(tempData.length+" <-"+ JSON.stringify(tempData))
      }
