...
export class UserManagementComponent implements OnInit {
  ...
  getData() {
    ...
    this.loginService.getUsers().subscribe(
      data => {
        ...
        tempUserList = data;
        let notDisplayAdmin = sessionStorage.getItem('currentUser')
        for(let item of tempUserList){
          if(item.status != 'reject' && item.username != sessionStorage.getItem('currentUser')){
  ...
  rejectUser(arrayIndex : number, message : string){
    ...
    console.log('->' + JSON.stringify(oldUser))
    this.loginService.updateRoleStatus(oldUser).subscribe();
    this.emailService.sendRejectionMessage(oldUser, message).subscribe();
    this.getData()
    // this.ngOnInit()
    this.router.navigate(['/usermanagement'])
  }
